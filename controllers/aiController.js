...
class AIController {
  ...
  // Per research (LlamaIndex CondenseQuestionChatEngine and friends), run ONE
  // cheap LLM condense pass whenever there is conversation history — with NO
  // content-based gate and no deterministic fast path. The prompt tells the
  // model to return the query verbatim when it is already self-contained.
  // Returns the raw query when nothing is needed (no history) or any stage
  // fails (never blocks the answer).
